...
deseq2plots = function (ct, ix, study, col.true, col.false, name.true, name.false, volcano.signatures = list(), v ,...) {
	factors.all <- factor( ix, levels = c(FALSE, TRUE), labels = c(name.false, name.true) )
	...
	pca.plot(log2(1+ct.test), col = ifelse(ix, col.true, col.false), pch = 19, ann.cex = 0.5)
	legend('topleft', c(name.true, name.false), col = c(col.true, col.false), pch = 19)
